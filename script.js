...
function compartilharProgresso() {
  ...
  if (navigator.share) { navigator.share({ title: 'Minha Evolução', text: texto }).catch(console.error); } 
  else { alert("Seu navegador não suporta o compartilhamento nativo. Copie o texto:\n\n" + texto); }
}
...
function compartilharTreinoModal() {
  ...
  if (navigator.share) { navigator.share({ title: 'Meu Treino', text: texto }).catch(console.error); } 
  else { alert("Seu navegador não suporta o compartilhamento nativo. Copie o texto:\n\n" + texto); }
}
...
function montarDadosParaIA() {
  ...
  const historicoPeso =
    JSON.parse(
      localStorage.getItem('historicoPeso') || '[]'
    );
  ...
  return {
    ...
    altura:
      localStorage.getItem('usuarioAltura'),

    metaPeso:
      localStorage.getItem('usuarioMeta') || "80.0",

    historicoPeso:
      historicoPeso.slice(-10),

    diarioHoje:
      historicoAlimentacao[hojeISO] || null,

    ultimosTreinos:
      historicoTreinos.slice(-5)
  };
}

function extrairBlocoIA(
  texto,
  inicio,
  fim
) {
  ...
  return texto
    .substring(corteInicio)
    .trim();
}

function exibirRespostaIA(
  plano,
  dica
) {
...
let isTreinando = false; let gpsWatchId = null; let timerInterval = null; let tempoInicio = 0; let distanciaTotalKm = 0; let ultimaPosicao = null;
let mapaTreino = null; let marcadorGPS = null; let rotaPolyline = null; let coordenadasRota = []; 
let mapaDetalhe = null; let detalhePolyline = null;

function iniciarMapaTreino() {
  if (mapaTreino !== null) { mapaTreino.invalidateSize(); return; }
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (pos) => { const latLng = [pos.coords.latitude, pos.coords.longitude]; mapaTreino.setView(latLng, 16); marcadorGPS = L.circleMarker(latLng, { color: '#0ea5e9', radius: 8, fillOpacity: 1 }).addTo(mapaTreino); },
...
function iniciarTreino() {
  ...
  document.getElementById('displayDistancia').innerText = "0.00"; document.getElementById('displayVelocidade').innerText = "0.0"; document.getElementById('displayCalorias').innerText = "0"; document.getElementById('displayTempo').innerText = "00:00:00";
  document.getElementById('btnIniciarTreino').style.display = 'none'; document.getElementById('btnPararTreino').style.display = 'block'; document.getElementById('msgGpsErro').style.display = 'none';
...
function processarPosicaoGps(posicao) {
  ...
  if (marcadorGPS) marcadorGPS.setLatLng(latLng); else marcadorGPS = L.circleMarker(latLng, { color: '#0ea5e9', radius: 8, fillOpacity: 1 }).addTo(mapaTreino);
  mapaTreino.setView(latLng);

  if (isTreinando) {
    coordenadasRota.push(latLng); rotaPolyline.setLatLngs(coordenadasRota); 
    ...
    ultimaPosicao = coordAtual; atualizarTelaTreino();
  }
}
...
function calcularDistanciaHaversine(pos1, pos2) {
  const R = 6371; const dLat = (pos2.lat - pos1.lat) * Math.PI / 180; const dLon = (pos2.lon - pos1.lon) * Math.PI / 180;
...
function atualizarTelaTreino() {
  ...
  const historico = obterHistorico(); const ordenado = [...historico].sort((a, b) => b.id - a.id); const pesoAtual = ordenado.length > 0 ? ordenado[0].peso : 80;
  const tipo = document.getElementById('tipoAtividade').value; let fatorCalorico = tipo === 'corrida' ? 1.03 : 0.75; 
...
function encerrarTreino() {
  if (!isTreinando) return;
  isTreinando = false; clearInterval(timerInterval); if (gpsWatchId) navigator.geolocation.clearWatch(gpsWatchId);
  document.getElementById('btnIniciarTreino').style.display = 'block'; document.getElementById('btnPararTreino').style.display = 'none';

  const treinos = JSON.parse(localStorage.getItem('historicoTreinos') || '[]');
  treinos.push({ id: Date.now(), data: new Date().toLocaleDateString('pt-BR'), tipo: document.getElementById('tipoAtividade').value, tempo: document.getElementById('displayTempo').innerText, distancia: document.getElementById('displayDistancia').innerText, calorias: document.getElementById('displayCalorias').innerText, rota: coordenadasRota });
  localStorage.setItem('historicoTreinos', JSON.stringify(treinos)); carregarHistoricoTreinos(); alert("Treino salvo com sucesso!");
}

function carregarHistoricoTreinos() {
  const treinos = JSON.parse(localStorage.getItem('historicoTreinos') || '[]');
  const lista = document.getElementById('historicoTreinosLista'); lista.innerHTML = '';
  const ordenado = [...treinos].sort((a, b) => b.id - a.id);
  ordenado.forEach(treino => {
    const icone = treino.tipo === 'corrida' ? '🏃' : '🚶'; const nome = treino.tipo.charAt(0).toUpperCase() + treino.tipo.slice(1);
    lista.innerHTML += `<div class="history-item" onclick="abrirModalTreino(${treino.id})"><div class="history-info"><span class="history-date">${icone} ${nome} - ${treino.data}</span><span class="history-medidas">⏱️ ${treino.tempo} | 🛣️ ${treino.distancia} km</span></div><div class="history-actions" style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px;"><span class="history-weight" style="color: #f97316;">🔥 ${treino.calorias} kcal</span><button class="btn-delete" onclick="event.stopPropagation(); deletarTreino(${treino.id})">Excluir</button></div></div>`;
  });
}
...
function deletarTreino(id) {
  ...
  const treinos = JSON.parse(localStorage.getItem('historicoTreinos') || '[]');
  localStorage.setItem('historicoTreinos', JSON.stringify(treinos.filter(t => t.id !== id))); carregarHistoricoTreinos();
}

function abrirModalTreino(id) {
  const treinos = JSON.parse(localStorage.getItem('historicoTreinos') || '[]'); const treino = treinos.find(t => t.id === id); if(!treino) return;
  ...
  document.getElementById('modalDistancia').innerText = treino.distancia; document.getElementById('modalTempo').innerText = treino.tempo; document.getElementById('modalCalorias').innerText = treino.calorias;
  document.getElementById('modalTreino').style.display = 'flex';

  setTimeout(() => {
    if (!mapaDetalhe) { mapaDetalhe = L.map('mapaDetalheContainer'); L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }).addTo(mapaDetalhe); detalhePolyline = L.polyline([], { color: '#ef4444', weight: 5 }).addTo(mapaDetalhe); }
    mapaDetalhe.invalidateSize();
    if(treino.rota && treino.rota.length > 0) { detalhePolyline.setLatLngs(treino.rota); mapaDetalhe.fitBounds(detalhePolyline.getBounds()); } 
    else { detalhePolyline.setLatLngs([]); mapaDetalhe.setView([-14.235, -51.925], 4); }
  }, 200);
}

function fecharModalTreino() { document.getElementById('modalTreino').style.display = 'none'; }
...
const itensPreProgramados = {
  cafe: ["Pão Francês", "Fruta", "Iogurte", "Café Preto", "Suco", "Ovos", "Biscoito"], almoco: ["Arroz", "Feijão", "Frango", "Carne", "Salada", "Legumes", "Macarrão"], jantar: ["Sopa", "Salada", "Frango", "Omelete", "Pão", "Iogurte", "Fruta"]
};
...
function carregarRefeicoesDoDia() {
  const dataSelect = document.getElementById('dataAlimentacaoInput').value; const historico = JSON.parse(localStorage.getItem('historicoAlimentacao') || '{}');
  if (historico[dataSelect]) { refeicoesAtuais = JSON.parse(JSON.stringify(historico[dataSelect])); if (typeof refeicoesAtuais.agua === 'undefined') refeicoesAtuais.agua = 0; } 
  else { refeicoesAtuais = { cafe: [], almoco: [], jantar: [], agua: 0 }; }
  renderizarTagsDeComida(); renderizarAgua();
}

function obterMetaAguaDinamica() {
  const historico = obterHistorico(); const ordenado = [...historico].sort((a, b) => b.id - a.id); const pesoAtual = ordenado.length > 0 ? ordenado[0].peso : 0;
  if (pesoAtual > 0) return Math.round(pesoAtual * 35); return 2000; 
}

function adicionarAgua(qtd) { refeicoesAtuais.agua += qtd; if (refeicoesAtuais.agua < 0) refeicoesAtuais.agua = 0; renderizarAgua(); }
...
function renderizarAgua() {
  ...
  const progressFill = document.getElementById('aguaProgressFill'); progressFill.style.width = `${porcentagem}%`; progressFill.style.backgroundColor = atual >= metaAgua ? '#10b981' : '#0ea5e9';
}
  ...
  ['cafe', 'almoco', 'jantar'].forEach(refeicao => {
    ...
    itensUnicos.forEach(item => {
      ...
      tag.addEventListener('click', function(event) {
        event.preventDefault(); const index = refeicoesAtuais[refeicao].indexOf(item);
        if (index > -1) refeicoesAtuais[refeicao].splice(index, 1); else refeicoesAtuais[refeicao].push(item); 
...
function adicionarComidaCustomizada(refeicao) {
  const input = document.getElementById(`custom-${refeicao}`); const valor = input.value.trim(); const valorFormatado = valor.charAt(0).toUpperCase() + valor.slice(1);
...
function salvarRefeicoes() {
  ...
  const btn = document.getElementById('btnSalvarAlimentacao'); const textoOriginal = btn.innerText; btn.innerText = "✅ Salvo com sucesso!"; btn.style.backgroundColor = "#10b981"; 
...
function carregarTema() {
  ...
  if (temaSalvo === 'dark') { document.documentElement.setAttribute('data-theme', 'dark'); document.getElementById('btnTema').innerText = '☀️ Claro'; document.getElementById('metaThemeColor').setAttribute('content', '#0f172a'); }
}

function alternarTema() {
  const temaAtual = document.documentElement.getAttribute('data-theme'); const metaColor = document.getElementById('metaThemeColor');
  if (temaAtual === 'dark') { document.documentElement.removeAttribute('data-theme'); localStorage.setItem('usuarioTema', 'light'); document.getElementById('btnTema').innerText = '🌙 Escuro'; metaColor.setAttribute('content', '#ffffff'); } 
  else { document.documentElement.setAttribute('data-theme', 'dark'); localStorage.setItem('usuarioTema', 'dark'); document.getElementById('btnTema').innerText = '☀️ Claro'; metaColor.setAttribute('content', '#0f172a'); }
  atualizarGraficos(); 
}
...
function salvarAltura() { const altura = parseFloat(document.getElementById('alturaInput').value); if (!altura || isNaN(altura) || altura <= 0) return alert('Altura inválida!'); localStorage.setItem('usuarioAltura', altura.toFixed(2)); verificarExibicaoAltura(); carregarDados(); }

function obterMeta() { return localStorage.getItem('usuarioMeta') || "80.0"; }
function carregarMeta() { const meta = obterMeta(); document.getElementById('pesoMetaCard').innerHTML = `${parseFloat(meta).toFixed(1)} <span style="font-size: 16px; font-weight: 400;">kg</span>`; document.getElementById('metaInput').value = meta; }
...
function salvarMeta() { const meta = parseFloat(document.getElementById('metaInput').value); if (!meta || isNaN(meta) || meta <= 0) return alert('Meta inválida!'); localStorage.setItem('usuarioMeta', meta.toFixed(1)); carregarMeta(); document.getElementById('cardMeta').style.display = 'none'; carregarDados(); }
...
function carregarDados() {
  ...
  if (altura && pesoAtual > 0) {
    ...
    if(imc < 18.5) document.getElementById('imcStatus').innerText = "Abaixo"; else if(imc < 25) document.getElementById('imcStatus').innerText = "Ideal"; else if(imc < 30) document.getElementById('imcStatus').innerText = "Sobrepeso"; else document.getElementById('imcStatus').innerText = "Obesidade";
  }
  ...
  if (historico.length > 0) {
    ...
    const dias = Math.round(Math.abs((new Date(cronologico[cronologico.length - 1].dataRaw) - new Date(cronologico[0].dataRaw)) / 86400000)); document.getElementById('tempoJornadaCard').innerText = dias === 0 ? "1º dia" : `${dias} dias`;
  }
...
function deletarRegistro(id) { if (!confirm("Excluir pesagem?")) return; localStorage.setItem('historicoPeso', JSON.stringify(obterHistorico().filter(i => i.id !== id))); carregarDados(); }
